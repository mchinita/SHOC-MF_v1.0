  ...
  do t=1,nadv
    ...
    if (do_edmf) then
       call integrate_mf(&
               ...
               plume_dry_height, cfl_mf)                 ! Output - 2D statistics of plume activation frequency
    else
       mf_dry_a = 0._rtype
       mf_dry_w = 0._rtype
       mf_dry_qt = 0._rtype
    ...
    call diag_second_shoc_moments(&
       ...
       w_sec,&                                ! Output
       mf_thlflx, mf_qtflx, &
       wthl_sec_ed,wthl_sec_mf, wqw_sec_ed, wqw_sec_mf)                   ! Output - EDMF)                   ! EDMF Output - diagnostic MF fluxes
    ...
    do k=2,nlev
      ...
      do i=1,shcol
        ...
        vertflux(i,k)=(-1._rtype*tkh_zi(i,k)*grid_dz*(invar(i,kt)-invar(i,k))) + &
          mf_aw_invar(i,k) - mf_aw(i,k)*0.5*(invar(i,kt)+invar(i,k)) 
          
        vertflux_ed(i,k)= -1._rtype*tkh_zi(i,k)*grid_dz*(invar(i,kt)-invar(i,k))
        
        vertflux_mf(i,k)= mf_aw_invar(i,k) - mf_aw(i,k)*0.5*(invar(i,kt)+invar(i,k)) 
